module deadcode.graphics.renderwindow;
...
alias WindowManagerSizeChange = void delegate();
WindowManagerSizeChange windowManagerSizeChangeHandler;
...
*/
	...
	//private static _disposeSetMousePositionMouseMoveEvent(Event e)
	//{
	//    auto support = (cast(void*)e)[0 .. typeid(MouseMoveEvent).initializer().length];
	//    destroy(support); 
	//    _mouseMoveEventAllocator.deallocate(support);
	//}
	//
	//void setMousePosition(Vec2f point) @nogc nothrow
	//{
	//    // need @nogc because it is called by SDL callback that needs to be @nogc
	//    try
	//    {
	//        import std.conv;
	//        void[] data = _mouseMoveEventAllocator.allocate(typeid(MouseMoveEvent).initializer().length);
	//        auto e = emplace!(MouseMoveEvent)(data, id, MouseID.max, cast(KeyMod)SDL_GetModState(),
	//                                          Vec2f(point.x, point.y), Vec2f(0, 0),
	//                                          cast(MouseButtonFlag)0);
	//        e.type = GUIEvents.mouseMove;
	//        e.disposeFunc = &_disposeSetMousePositionMouseMoveEvent;
	//        _eventSink.putAsOwnerThreadIfRoom(e);
	//    }
	//    catch (Throwable)
	//    {
	//        assert(0);
	...
	@property
	{
        ...
        override bool visible() const 
        {
            return (SDL_GetWindowFlags(cast(SDL_Window*)win) & SDL_WINDOW_SHOWN) != 0;
        }
        
        override void visible(bool v) 
        {
            if (v)
                SDL_ShowWindow(win);
            else
